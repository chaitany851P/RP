...
\subsection{Training Convergence Dynamics and Backbone Detection}

The network was trained across 100 epochs using SGD ($\beta = 0.937$, $\lambda = 0.0005$, $\eta_0 = 0.01$) with cosine annealing decay. Fig.~\ref{fig:train_curves} illustrates empirical training and validation loss curves alongside metric convergence. Bounding box regression loss decreases to 0.41, classification loss drops to 0.28, and distribution focal loss (DFL) declines to 0.83, confirming sharp localization without overfitting. Precision and recall plateau at 0.947 and 0.926, while mAP@0.5 reaches 0.968.

\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig2_training_curves.png}
\caption{Empirical training and validation convergence dynamics across 100 epochs on the industrial benchmark dataset. (A) Bounding-box regression loss (`train/box\_loss` vs. `val/box\_loss`), (B) Object classification loss (`train/cls\_loss` vs. `val/cls\_loss`), (C) Distribution Focal Loss (`train/dfl\_loss` vs. `val/dfl\_loss`), (D) Precision and Recall progression (`metrics/precision(B)` and `metrics/recall(B)`), (E) Mean Average Precision trajectory at IoU thresholds 0.50 and 0.50:0.95 (`metrics/mAP50` and `metrics/mAP50-95`), and (F) Cosine annealing learning rate decay schedule.}
\label{fig:train_curves}
\end{figure*}

Table~\ref{tab:backbones} delineates comparative performance across YOLOv8 variants. While YOLOv8l attained 0.971 mAP@0.5, YOLOv8n was chosen as the operational baseline due to its minimal parameter footprint (3.2 M) and 4.2 ms inference latency, leaving compute margin for concurrent pose and activity heads.
...
\subsection{Multi-Hazard Detection Fidelity and Confusion Matrix}
Table~\ref{tab:hazards} summarizes validation metrics across 25 industrial video scenarios comprising staged occupational hazards. The aggregate pipeline achieved an F1-score of 0.936 and an overall mAP@0.5 of 0.968. Danger zone breaches recorded the highest fidelity ($F_1 = 0.983$) due to the mathematical precision of the signed distance polygon test.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/fig3_confusion_matrix.png}
\caption{Normalized confusion matrix heatmap across 8 industrial safety states evaluated on test video sequences. High diagonal values confirm minimal cross-class confusion.}
\label{fig:conf_matrix}
\end{figure}

Fig.~\ref{fig:conf_matrix} details the normalized confusion matrix across 8 operational states. True positive rates along the diagonal reach 0.98 for danger zone entry, 0.97 for fall detection, and 0.97 for normal activity. Minor confusions (0.04--0.05) occur between loitering and roaming due to trajectory boundary transitions across $D_{\text{roam}} = 150\text{ px}$.

\subsection{Operating Thresholds and Precision-Recall Dynamics}

Fig.~\ref{fig:f1_curve} depicts the F1-score as a function of detection confidence threshold across hazard classes. The macro-average curve peaks at $F_1 = 0.94$ at $\tau_{\text{conf}} = 0.42$, confirming $\tau_{\text{conf}} = 0.40$ as an optimal operational setting for factory floor deployment.

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/fig4_f1_confidence_curve.png}
\caption{F1-score as a function of detection confidence threshold across individual hazard classes. The bold curve denotes the macro-average performance, displaying an optimal peak operating point of $F_1 = 0.94$ at confidence threshold $\tau_{\text{conf}} = 0.42$.}
\label{fig:f1_curve}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/fig5_pr_curve.png}
\caption{Precision-Recall (PR) operating curves across all individual hazard classes. The aggregate system achieves 0.968 mAP@0.5.}
\label{fig:pr_curves}
\end{figure}

Fig.~\ref{fig:pr_curves} graphs Precision-Recall dynamics across classes. Danger zone entry sustains 0.985 mAP@0.5 and fall detection reaches 0.975 mAP@0.5, with the aggregate system achieving 0.968 mAP@0.5.

\subsection{Computational Latency and Real-Time Budget}
Table~\ref{tab:latency} delineates execution latency across processing stages. On GPU hardware, the complete pipeline achieves 28.5 FPS, satisfying the 25--30 FPS standard for real-time video surveillance.
...
To analyze suppression of transient false alarms, Table~\ref{tab:ablation} evaluates the trade-off between False Alarm Rate (FAR) and reaction latency as $N_{\text{confirm}}$ varies from 1 to 10 frames. Setting $N_{\text{confirm}} \in [5, 6]$ suppresses FAR below 0.5\% with sub-quarter-second latency.
...
\subsection{Qualitative Detection Results on Shop-Floor Video}

Fig.~\ref{fig:qualitative} presents qualitative detection results on real test frames from factory video feeds:
(A) Ground-level slip/fall incident ($AR = 2.14$), (B) Unconscious collapse adjacent to operating machinery with active alert banner, (C) Multi-worker safe zone tracking with ByteTrack IDs, (D) Restricted perimeter breach with red polygonal highlight, (E) Forbidden mobile phone distraction inside active machine cell, and (F) Deep squatting posture strain ($\theta_{\text{knee}} < 100^\circ$).

\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig6_qualitative_detections.png}
\caption{Qualitative detection and multi-hazard visual analytics demonstrated on real industrial test frames from the benchmark dataset. (A) Ground-level slip/fall detection with high aspect ratio ($AR = 2.14$) and bounding-box overlay, (B) Unconscious worker collapse under machinery with critical emergency alert banner, (C) Multi-worker safe zone monitoring with active ByteTrack tracklets, (D) Restricted danger perimeter breach ($N_{\text{in}} \ge 3$) with red polygonal highlight, (E) Forbidden secondary activity (mobile phone distraction while walking) flagged inside active machine cell, and (F) Awkward posture and deep squatting ($\theta_{\text{knee}} < 100^\circ$) flagged by the kinematic pose estimator.}
